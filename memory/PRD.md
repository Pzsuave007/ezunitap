...
- **Feb 2026 (Landing — narrativa de valor / venta)** — El usuario pidió que la landing comunique el "valor gigante" del producto (no solo features). Estrategia acordada: vender la transformación + el flujo conectado como motor de crecimiento (flywheel). Se agregaron **4 secciones nuevas a `Landing.js` SIN tocar lo existente** (se irán podando después): (1) **Antes vs Después** — split rojo (sin Unitap) vs gradiente (con Unitap). (2) **"La historia de un trabajo"** (`#historia`) — walkthrough de 6 pasos en tarjetas con flechas conectándolas (Te encuentran → Cotizas → Firman → Agendas → Cobras → Reseña 5★) + callout "flywheel" (cada reseña trae el siguiente cliente). (3) **El costo de NO tenerlo** — sección oscura, loss aversion (3 tarjetas). (4) **Ancla de precio** — $800 (perder un trabajo) vs $49/mes. Nuevo link de nav "La historia". Decisiones por defecto: sin testimonios inventados (sitio en prod real), cifras de ejemplo realistas, persona "Carlos – Landscaping". Lint limpio, secciones `#historia`/`#transformacion` confirmadas en DOM. Build prod regenerado + `git add -f frontend/build/`. **Pendiente: revisión visual del usuario.** Files: `/app/frontend/src/pages/Landing.js`.
