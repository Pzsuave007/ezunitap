...
- **Feb 2026 (Comp Accounts)** — Added super-admin "Cuentas gratis" feature so the owner can give free accounts to friends, beta-testers, reviewers. Two flows: (1) generate single-use invite link `?invite=TOKEN` that auto-grants comp on signup, (2) toggle comp on any existing user. Optional duration (30/90/180/365 days or indefinite) + internal note. Comp users get `is_comp=true`, `subscription_status='active'`, `plan_type='comp'`, full Smart Card unlock. Backend: `RegisterIn.invite_token`, endpoints `POST/GET/DELETE /api/admin/comp-invites`, `GET /api/admin/users`, `POST /api/admin/users/{id}/{grant-comp,revoke-comp}`. New collection `comp_invites`. SUPER_ADMIN_EMAIL changed to `pzsuave007@gmail.com`. Frontend: `/admin/cuentas` page with Invitaciones + Usuarios tabs, copy/share buttons, Register page detects `?invite=` and shows green gift banner, TrialBanner hidden for comp users. **Tested 22/22 backend + 100% frontend** (iteration_10.json). Files: `/app/backend/server.py` (admin section + RegisterIn + register), `/app/backend/payments_service.py` (is_comp honored), `/app/frontend/src/pages/AdminAccounts.js`, `Register.js`, `/app/frontend/src/components/{Layout,TrialBanner}.js`, `/app/backend/tests/test_comp_invites.py` (22 cases).
