...
- **Frontend forms**: toggle "Dirigir invoice solo al nombre de la compañía" (verde=ON) que aparece SOLO si el cliente tiene `company` lleno. En `ClientForm.js` (crear, testid `cli-company-only-toggle`) y `ClientDetail.js` (editar, testid `client-company-only-toggle`). i18n `clients.companyOnly/companyOnlyHint` (ES/EN); ClientDetail usa texto ES directo (archivo es ES-only).
- **Render Bill To** (cuando `bill_to_company_only && company`): muestra SOLO la compañía, oculta nombre/Attn:
  - `PublicInvoice.js`: oculta la línea "Attn: {name}" (el primario ya era `company||name`).
  - `PublicQuote.js` y `PublicAgreement.js`: muestran `company` en vez de `name`.
  - `lib/pdf.js` `clientBlock` (compartido por invoice/quote/agreement PDF): omite la línea "Attn: {person}".
- **Verificado**: cliente "Juan Perez" / "ACME LLC" con flag ON → invoice público muestra "BILL TO: ACME LLC" + email, SIN "Juan Perez" ni "Attn" (screenshot + curl del payload `bill_to_company_only=True`). Datos de prueba borrados.
- Build main.19daa87c.js (con fix oxlint-disable en chunks vendor, ver `/app/memory/oxlint_build_fix.md`) + `git add -f frontend/build`. ⚠️ DESPLIEGUE: **backend + frontend** → "Save to GitHub" + `git pull && bash deploy.sh`.


## 🙈💲 Jun 2026 — Ocultar nombre del dueño en invoices (toggle) + corrección precio Presencia a $75 [COMPLETO; verificado curl+screenshot, SIN testing_agent]
- **A) Ocultar nombre del dueño**: el cliente quiere que en documentos salga solo el nombre de la COMPAÑÍA, no su nombre personal. Nuevo toggle en Ajustes (Settings.js, "Mostrar mi nombre en cotizaciones e invoices", testid `hide-owner-name-toggle`) → campo `hide_owner_name` en `BusinessUpdate` (persistido vía PUT /auth/me). El payload de invoice ya incluye el user completo como `business`, así que trae `hide_owner_name`. Frontend `PublicInvoice.js`: oculta `business.owner_name` si `hide_owner_name`. (El PDF y quotes/contratos ya usaban solo `business_name`, no requieren cambio.) i18n `profile.showOwnerName/Hint` (ES/EN). Verificado: con flag ON el invoice público muestra solo "Uni2 Marketing Agency", sin "Paul Zacapantzi".
- **B) Corrección de precio**: Presencia Digital $54.99 → **$75** (dueño lo pidió). Combos recalculados −30%: presencia+negocio **$80.99**, presencia+marketing **$73.99**. Bundle sigue **$99.99**, fundador **$75**, negocio+marketing $48.99. Ahorro del bundle actualizado a **$44.99** (144.97−99.99). Backend `payments_service.py` + Landing.js price string + i18n bundleDesc. Verificado: PLANS display + screenshot landing (Presencia $75, bundle $99.99, "Save $44.99 a month").
- Nota: texto del bundle actualizado a "Ahorra $540 al año — 3 planes por el precio de 2" (ES) / "Save $540 a year — 3 plans for the price of 2" (EN) para mayor atractivo. Build main.c7580f9c.js.
- Build main.cc39a300.js + `git add -f frontend/build/*`. ⚠️ DESPLIEGUE: **backend + frontend** → "Save to GitHub" + `git pull && bash deploy.sh`.


- **Petición dueño**: si el invoice pide 50% down (o monto específico), que el cliente SOLO pueda pagar eso, el sistema lo marque como pagado parcial y deje el resto para después. Antes el checkout cobraba todo.
- **Decisiones**: (1a) solo botón del enganche hasta cubrirlo, luego "pagar saldo"; (2a) el enganche sale del quote/acuerdo/ajustes (ya existía `deposit_amount`); (3a) queda "Parcial" con saldo visible, se puede pagar luego.
- **Backend** (`server.py`): `public_invoice` ahora expone en `card_payment`: `deposit_due` (= min(deposit_amount − amount_paid, remaining)) y `deposit_amount`. `InvoiceCheckoutIn` acepta `pay_deposit: bool`; en `public_invoice_checkout`, si `pay_deposit` y hay `deposit_due>0`, cobra SOLO ese monto (validado server-side, no confía en el cliente). `_recalc_invoice_payments` marca `deposit_paid` cuando amount_paid ≥ deposit_amount (el estado "partial" ya existía). El pago por tarjeta entra al ledger vía webhook → recalcula estado/saldo.
- **Frontend** (`PublicInvoice.js` `StripeCardPay`): si `deposit_due>0 && < remaining` → modo enganche: título "Pay your deposit", texto del monto, nota "Remaining balance $X stays due and can be paid later", botón "Pay deposit $X now" (envía `pay_deposit:true`). Si no, paga el saldo total como antes. testid `deposit-remaining-note`.
- **Verificado**: invoice INV-2008 (total $4,340, enganche $1,500) → payload `deposit_due=1500`; checkout `pay_deposit:true` crea sesión Stripe por **$1,500** (150000c), `pay_deposit:false` por $4,340; screenshot público muestra "Pay deposit $1,500.00 now" + saldo $2,840 pendiente.
- Build main.0c303984.js + `git add -f frontend/build/*`. ⚠️ DESPLIEGUE: **backend + frontend** → "Save to GitHub" + `git pull && bash deploy.sh`.


- **Petición dueño**: en los 9 pasos faltaba mencionar en el PASO 1 (captar clientes nuevos) cómo la página web y las landing pages de servicios ayudan a conseguir clientes.
- **Fix** (i18n `landing.flow[0]` ES/EN): el paso "Consigue nuevos clientes / Win new clients" ahora lidera con "Te encuentran en Google con tu página web y una landing page que convierte para cada servicio…" + bullets: web profesional + landing pages por servicio, te encuentran en Google, ven servicios/fotos, piden cotización/guardan contacto, todo automático al sistema.
- **Verificado screenshot**: Step 01 muestra el nuevo copy. Build main.8310e368.js + `git add -f frontend/build/*`. ⚠️ DESPLIEGUE: solo frontend → "Save to GitHub" + `git pull && bash deploy.sh`.


- **Petición dueño**: el home no mencionaba la web completa ni las landing pages que convierten; actualizarlo para reflejar los features nuevos. Meter el Website al plan **Presencia Digital** (no plan aparte) y subir el precio. Además: el Website es un feature clave para la presencia digital y para salir en Google → destacarlo fuerte.
- **Precios** (`backend/payments_service.py`, cobro real Stripe): presencia $34.99→**$54.99**; combos con presencia recalculados −30%: presencia+negocio **$66.99**, presencia+marketing **$59.99** (negocio+marketing sin cambio $48.99); bundle "Todo UniTech" $75→**$99.99**; oferta fundador $59→**$75/mes** (regular mostrado ahora $99.99). Anual = x10 (automático). Tagline de presencia actualizado. Verificado por import de PLANS.
- **Landing** (`Landing.js` + i18n ES/EN): (1) card de **Presencia Digital** reescrita (web completa 10 diseños + landing pages que convierten + galería Recent Work + Google) y precio $54.99; (2) oferta fundador $75 / regular $99.99 (usa `founder.display_price`), bundle $99.99, ahorro bundle ~$25; (3) hero y sección NFC ahora mencionan web + landing pages; (4) **NUEVA sección dedicada `#website`** (`landing-website-showcase`): eyebrow "Tu página web", título "…te consigue clientes en Google", mock de navegador (hero de conversión + chips "Problems we solve"), badge flotante "Get found on Google Search", 6 bullets (10 diseños, landing pages IA, SEO, Recent Work que vende, captura leads/citas, dominio propio) + CTA "Crear mi página web". Nuevas claves i18n `landing.website*`.
- **Verificado**: screenshot de la sección (fuerte y prominente) + card Presencia con $54.99; JSON válido; precios backend correctos.
- Build main.8d7058e3.js + `git add -f frontend/build/*`. ⚠️ DESPLIEGUE: **backend + frontend** → "Save to GitHub" + `git pull && bash deploy.sh`.
