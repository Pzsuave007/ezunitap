...
- **Feb 2026 (UX Simplification)** — Reorganized sidebar following the natural business flow: Inicio → Clientes → Quotes → Contratos → Invoices → Agenda → Trabajos, then a separator, then account items (Tarjeta, Perfil, Suscripción, Admin). Removed `Mensajes AI` and `Scope of Work` from the main menu (routes still work directly). Moved them into the Client profile: added 6 action buttons (Quote AI, Contrato, Invoice, Mensaje, Scope, Foto) and 6 tabs (Info, Quotes, Contratos NEW, Invoices, Mensajes, Fotos). New `ClientScopeDialog` component for inline AI scope generation pre-filled with client.job_type + notes. Auto-create Job: when `POST /api/invoices/{id}/status?status=paid` is called, a new Job is automatically created in `db.jobs` (status='approved', invoice_id linked, auto_created=true, idempotent by invoice_id). `client_history` endpoint extended with `agreements` and `scopes` arrays. **Tested 7/7 backend + 100% frontend (iteration_11.json)**. Files: `/app/frontend/src/components/Layout.js`, `/app/frontend/src/pages/ClientDetail.js`, `/app/frontend/src/components/ClientScopeDialog.js`, `/app/backend/server.py` (client_history + set_invoice_status), `/app/backend/tests/test_nav_and_autojob.py`.
