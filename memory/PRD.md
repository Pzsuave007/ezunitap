...
- **Feb 2026** — Pipeline automático completo: (a) cliente acepta quote desde link público con botón "Accept this Quote" → respuesta instantánea (110ms) y agreement se genera en `BackgroundTask`. (b) Al firmar el contrato, si tiene `quote_id`, se auto-crea invoice en `draft` con line items copiados (+ `quote_id` y `agreement_id` linkeados). Sin envío automático — el contratista copia y manda links manualmente por WhatsApp/SMS/email. Tested 100% (18/18 backend incluyendo regresión, + E2E pipeline completo). Files: `/app/backend/server.py` (public_accept_quote + _auto_create_agreement_from_quote + auto-invoice en public_sign_agreement), `/app/frontend/src/pages/PublicQuote.js` (botón Accept).
- **Feb 2026** — AI Service Agreement Generator: contratos legales en inglés (paralegal-grade) generados con GPT-5.2, firma digital con 2 modos (canvas con dedo + botón "I Accept"), auto-crea contrato cuando un Quote pasa a `approved` (idempotente). Endpoints `/api/ai/agreement`, CRUD `/api/agreements`, públicos `/api/public/agreements/:id` + `/sign`. Cláusulas adaptativas por industria (la AI detecta lawn care, painting, plumbing, etc. y agrega cláusulas específicas). Tested 100% (10/10 backend + frontend e2e). Files: `/app/frontend/src/pages/{Agreements,AgreementBuilder,AgreementDetail,PublicAgreement}.js`, `/app/frontend/src/components/SignaturePad.js`, `/app/backend/ai_service.py` (`generate_service_agreement`), `/app/backend/server.py` (sección "SERVICE AGREEMENTS"), `/app/backend/tests/test_agreements.py`.
