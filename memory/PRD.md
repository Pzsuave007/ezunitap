...
- **Jun 18 2026 (iOS Safari invoice inputs — completado)** — Cierre del fix de edición de facturas en iPhone/Safari. Además de los line items (cantidad/precio, ya arreglados), se convirtieron los campos restantes que aún usaban `type="number"` (Tax Rate, Depósito/Down payment, y montos del plan de pago) a `type="text" inputMode="decimal"` con sanitización vía helper `numClean` (movido a nivel de módulo para que sea accesible desde `PaymentPlanEditor`). Coerción string→Number en `save()` para `tax_rate` y `deposit_amount`. Webpack compila limpio; verificado por screenshot (6 inputs decimales detectados en la factura). **Pendiente: el usuario debe subir/desplegar el build y validar en su iPhone.** File: `/app/frontend/src/pages/InvoiceDetail.js`.
