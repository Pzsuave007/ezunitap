...
export default function ClientDetail() {
  ...
  const td = (k, opts) => t("clients.detail." + k, opts);
  const dateLocale = i18n.language === "en" ? "en-US" : "es";
  ...
  const preferredContact = (() => {
    ...
    const map = {
      ...
      text: { label: td("sms"), href: client.phone ? `sms:${client.phone}` : null },
      sms: { label: td("sms"), href: client.phone ? `sms:${client.phone}` : null },
      email: { label: "Email", href: client.email ? `mailto:${client.email}` : null },
      phone: { label: td("phoneCall"), href: client.phone ? `tel:${client.phone}` : null },
    };
    ...
    <Card data-testid="client-lead-card" className="card-elevated p-4 border-0 shadow-none bg-amber-50 ring-1 ring-amber-200">
      <div className="flex items-center justify-between gap-2 mb-2.5">
        <div className="flex items-center gap-2 font-heading font-bold text-sm text-amber-900">
          {fromWeb ? td("leadFromWebsite") : td("leadFromCard")}
        </div>
        <span
        ...
        >
          {client.lead_type === "connect" ? td("wantsConnect") : td("askedQuote")}
        </span>
      </div>

      {fromWeb && client?.source_site && (
        <div className="mb-2.5 text-xs text-amber-800" data-testid="client-source-site">
          {td("cameFrom")} <span className="font-semibold">{client.source_site}</span>
        </div>
      )}

      {interests.length > 0 && (
        <div className="mb-2.5">
          <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1">{td("interestedIn")}</div>
      ...
      {preferredContact && (
        <div className="mb-2.5 flex items-center justify-between gap-2 bg-white rounded-xl px-3 py-2 ring-1 ring-amber-200">
          <div className="min-w-0">
            <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">{td("prefersContact")}</div>
      ...
      {client.project_request && (
        <div className="mb-2.5">
          <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1">{td("theirMessage")}</div>
      ...
      {projectPhoto && (
        <a href={projectPhoto} target="_blank" rel="noreferrer" className="block mb-2.5">
          <img data-testid="client-project-photo" src={projectPhoto} alt={td("projectPhotoAlt")}
      ...
      {hasBusiness && (
        <Button data-testid="project-create-quote-btn" onClick={startAiQuote}
          ...
          <Sparkles className="w-4 h-4 mr-2" /> {td("createQuoteFromThis")}
        </Button>
      )}
    </Card>
    );
  ...
  const del = async () => {
    if (!window.confirm(td("confirmDelete"))) return;
  ...
  return (
    <div className="max-w-3xl mx-auto w-full pb-10 space-y-7">
      <button onClick={() => navigate("/clientes")} className="flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-800 transition-colors tap" data-testid="back-to-clients">
        <ArrowLeft className="w-4 h-4" /> {td("back")}
      </button>
      ...
      {(client.phone || client.email || client.address) && (
        <div className="flex items-start justify-center gap-6">
          {client.phone && (
            <ContactCircle href={`tel:${client.phone}`} icon={Phone} label={td("call")} testid="contact-call-button" />
          )}
          {client.email && (
            <ContactCircle href={`mailto:${client.email}`} icon={Mail} label="Email" testid="contact-email-button" />
          )}
          {client.address && (
            <ContactCircle href={`https://maps.google.com/?q=${encodeURIComponent(client.address)}`} external icon={MapPin} label={td("map")} testid="contact-map-button" />
          )}
        </div>
      )}
        ...
        <DrawerTrigger asChild>
          <Button data-testid="fab-nuevo-button" className="w-full h-auto py-2.5 rounded-2xl bg-zinc-900 hover:bg-zinc-800 text-white shadow-sm transition-colors flex-col gap-0.5">
            <span className="flex items-center text-base font-semibold leading-none">
              <Plus className="w-5 h-5 mr-1.5" /> {td("create")}
            </span>
            <span className="text-[11px] font-medium text-zinc-300 leading-none">{td("createSubtitle")}</span>
          </Button>
        </DrawerTrigger>
        <DrawerContent>
          <DrawerHeader className="pb-1">
            <DrawerTitle className="font-heading">{td("createFor", { name: client.name?.split(" ")[0] })}</DrawerTitle>
          </DrawerHeader>
          <div className="p-4 pt-1 pb-8 max-w-md mx-auto w-full space-y-2">
            <ActionRow icon={Sparkles} iconCls="bg-emerald-100 text-emerald-700" title={td("aiQuote")} desc={td("aiQuoteDesc")} testid="drawer-action-quote-ai"
              onClick={startAiQuote} />
            <ActionRow icon={Receipt} iconCls="bg-blue-100 text-blue-800" title={td("invoice")} desc={td("invoiceDesc")} testid="drawer-action-invoice"
              onClick={() => navigate(`/invoices/nuevo?client_id=${id}`)} />
            <ActionRow icon={FileSignature} iconCls="bg-violet-100 text-violet-700" title={td("contract")} desc={td("contractDesc")} testid="drawer-action-contract"
              onClick={() => navigate(`/contratos/nuevo?client_id=${id}`)} />
            <ActionRow icon={MessageSquare} iconCls="bg-sky-100 text-sky-700" title={td("message")} desc={td("messageDesc")} testid="drawer-action-message"
              onClick={() => navigate(`/mensajes?client_id=${id}`)} />
            <ActionRow icon={Sparkles} iconCls="bg-amber-100 text-amber-600" title={td("aiScope")} desc={td("aiScopeDesc")} testid="drawer-action-scope"
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <span className="min-w-0 flex-1 text-left">
              <span className="block font-semibold text-sm">{td("sendMessage")}</span>
              <span className="block text-[11px] text-zinc-300">{td("sendMessageDesc")}</span>
            </span>
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block font-semibold text-zinc-900 text-sm">{td("upsellNegocio")}</span>
              <span className="block text-xs text-zinc-500">{td("upsellNegocioDesc")}</span>
            </span>
      ...
      <div className="grid grid-cols-2 border border-zinc-200 rounded-2xl overflow-hidden bg-white">
        <StatCell label={td("statQuotes")} value={history.quotes.length} testid="stats-quotes-value" cls="border-r border-b border-zinc-100" />
        <StatCell label={td("statInvoices")} value={history.invoices.length} testid="stats-invoices-value" cls="border-b border-zinc-100" />
        <StatCell label={td("statInvoiced")} value={fmtMoney(totalInvoiced)} testid="stats-invoiced-value" cls="border-r border-zinc-100" />
        <StatCell label={td("statPending")} value={fmtMoney(pending)} testid="stats-pending-value" valueCls="text-amber-600" />
      </div>
      )}
      ...
      <Tabs defaultValue="info" className="w-full">
        <TabsList className="w-full justify-start gap-1 bg-zinc-100 p-1 rounded-xl overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden h-auto">
          <Seg value="info" label={td("tabInfo")} testid="history-tab-info" />
          <Seg value="notes" label={td("tabNotes")} count={notes.length} testid="history-tab-notas" />
          {hasBusiness && <Seg value="quotes" label={td("tabQuotes")} count={history.quotes.length} testid="history-tab-cotizaciones" />}
          {hasBusiness && <Seg value="agreements" label={td("tabContracts")} count={history.agreements.length} testid="history-tab-contratos" />}
          {hasBusiness && <Seg value="invoices" label={td("tabInvoices")} count={history.invoices.length} testid="history-tab-facturas" />}
          {hasBusiness && <Seg value="jobs" label={td("tabJobs")} count={history.jobs.length} testid="history-tab-trabajos" />}
          <Seg value="messages" label={td("tabMessages")} count={history.messages.length} testid="history-tab-mensajes" />
          {hasBusiness && <Seg value="photos" label={td("tabPhotos")} count={history.photos.length} testid="history-tab-fotos" />}
        </TabsList>
        ...
        <TabsContent value="info" className="mt-4 space-y-4">
          ...
          <Card className="card-elevated p-5 border-0 shadow-none">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-heading font-bold">{td("information")}</h3>
              {!editing ? (
                <div className="flex gap-2">
                  <Button data-testid="edit-client" size="sm" variant="outline" onClick={() => setEditing(true)} className="rounded-xl">{td("edit")}</Button>
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => { setEditing(false); setForm(client); }} className="rounded-xl">{t("common.cancel")}</Button>
                  <Button size="sm" data-testid="save-client" onClick={save} className="rounded-xl bg-emerald-600 hover:bg-emerald-700">{t("common.save")}</Button>
                </div>
              )}
            </div>
            {editing ? (
              <div className="space-y-3">
                {[
                  ["name", td("fieldName")], ["company", td("fieldCompany")],
                  ["phone", td("fieldPhone")], ["email", td("fieldEmail")],
                  ["address", td("fieldAddress")], ["job_type", td("fieldJobType")],
                ].map(([k, label]) => (
                  <div key={k}>
                    <Label>{label}</Label>
                ...
                {form.company?.trim() && (
                  <div className="flex items-start justify-between gap-4 rounded-xl border border-zinc-200 bg-zinc-50 p-3" data-testid="client-company-only-setting">
                    <div className="flex-1">
                      <Label className="cursor-pointer">{td("companyOnlyLabel")}</Label>
                      <p className="text-[11px] text-zinc-500 mt-1">{td("companyOnlyHint")}</p>
                    </div>
                    <button
                      type="button"
        ...
        <TabsContent value="notes" className="mt-4 space-y-3">
          <Card className="card-elevated p-4 border-0 shadow-none">
            <Label className="text-xs font-bold uppercase tracking-wider text-zinc-400">{td("newNote")}</Label>
            <Textarea
              data-testid="note-input"
              value={noteText}
              onChange={(e) => setNoteText(e.target.value)}
              placeholder={td("newNotePlaceholder")}
              rows={3}
            ...
            >
              {noteSaving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Plus className="w-4 h-4 mr-2" />} {td("addNote")}
            </Button>
          </Card>
          ...
          {notes.length === 0 ? (
            <Card className="card-elevated p-6 text-center border-0 shadow-none">
              ...
              <p className="text-sm text-zinc-500">{td("noNotesYet")}</p>
              <p className="text-xs text-zinc-400 mt-1">{td("noNotesHint")}</p>
            </Card>
          ) : (
            notes.map((n) => (
              <Card
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-zinc-400 mt-1.5">{new Date(n.created_at).toLocaleString(dateLocale)}</div>
                  </div>
                  <button
        ...
        <TabsContent value="quotes" className="mt-4 space-y-2">
          {history.quotes.length === 0 ? <EmptyHist label={td("emptyHist", { label: td("labelQuotes") })} /> : history.quotes.map((q) => (
        ...
        <TabsContent value="agreements" className="mt-4 space-y-2">
          {history.agreements.length === 0 ? <EmptyHist label={td("emptyHist", { label: td("labelContracts") })} /> : history.agreements.map((a) => (
            <Card
              key={a.id}
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-semibold truncate">{a.title || a.job_title || td("contractDefault")}</div>
                  <div className="text-xs text-zinc-500">{new Date(a.created_at).toLocaleDateString(dateLocale)}</div>
                </div>
                <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full ${
                  ...
                  {a.status === "signed" ? td("statusSigned") :
                   a.status === "sent" ? td("statusSent") :
                   a.status === "draft" ? td("statusDraft") : a.status}
                </span>
              </div>
            </Card>
          ))}
        ...
        <TabsContent value="invoices" className="mt-4 space-y-2">
          {history.invoices.length === 0 ? <EmptyHist label={td("emptyHist", { label: td("labelInvoices") })} /> : history.invoices.map((i) => (
        ...
        <TabsContent value="jobs" className="mt-4 space-y-2">
          {history.jobs.length === 0 ? <EmptyHist label={td("emptyHist", { label: td("labelJobs") })} /> : (() => {
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-zinc-500 mt-0.5">
                      {j.scheduled_date
                        ? new Date(j.scheduled_date + "T00:00:00").toLocaleDateString(dateLocale, { day: "numeric", month: "short", year: "numeric" })
                        : (j.created_at ? new Date(j.created_at).toLocaleDateString(dateLocale) : "")}
                    </div>
                  </div>
        ...
        <TabsContent value="messages" className="mt-4 space-y-2">
          {history.messages.length === 0 ? <EmptyHist label={td("emptyHist", { label: td("labelMessages") })} /> : history.messages.map((m) => (
            <Card key={m.id} className="card-elevated p-4 border-0 shadow-none">
              ...
              <div className="text-xs text-zinc-400 mt-2">{new Date(m.created_at).toLocaleString(dateLocale)}</div>
            </Card>
          ))}
        </TabsContent>
        ...
        <TabsContent value="photos" className="mt-4">
          {history.photos.length === 0 ? (
            <Card className="card-elevated p-6 text-center border-0 shadow-none">
              ...
              <p className="text-sm text-zinc-500">{td("noPhotos")}</p>
              <p className="text-xs text-zinc-400 mt-1">{td("noPhotosHint")}</p>
            </Card>
          ) : (
      ...
      <Drawer open={!!activeNote} onOpenChange={(o) => !o && setActiveNote(null)}>
        <DrawerContent>
          <DrawerHeader className="pb-1">
            <DrawerTitle className="font-heading">{td("noteActionTitle")}</DrawerTitle>
          </DrawerHeader>
          {activeNote && (
          ...
          <div className="p-4 pt-3 pb-8 max-w-md mx-auto w-full space-y-2">
            {hasBusiness && (
              <ActionRow icon={Sparkles} iconCls="bg-emerald-100 text-emerald-700" title={td("noteCreateQuote")} desc={td("noteCreateQuoteDesc")} testid="note-action-quote"
                onClick={() => noteAction("quote")} />
            )}
            {hasBusiness && (
              <ActionRow icon={Receipt} iconCls="bg-blue-100 text-blue-800" title={td("noteCreateInvoice")} desc={td("noteCreateInvoiceDesc")} testid="note-action-invoice"
                onClick={() => noteAction("invoice")} />
            )}
            <ActionRow icon={MessageSquare} iconCls="bg-sky-100 text-sky-700" title={td("noteSendMessage")} desc={td("noteSendMessageDesc")} testid="note-action-message"
